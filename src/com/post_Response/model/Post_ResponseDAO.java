...
public class Post_ResponseDAO implements Post_ResponseDAO_interface{
	...
	// 一個應用程式中,針對一個資料庫 ,共用一個DataSource即可
		...
		private static final String INSERT_STMT = 
				"INSERT INTO post_Response (res_Id,mem_Id,post_Id,post_Response_content,post_time,post_Response_upDate) VALUES (post_Response_seq1.NEXTVAL, ?, ?, ?, ?, ?)";
		private static final String GET_ALL_STMT = 
				"SELECT res_Id,mem_Id,post_Id,post_Response_content,post_time,post_Response_upDate FROM post_Response";
		...
		private static final String GET_Posts_ByPost_Id_STMT =
				"SELECTpost_Id,mem_Id,post_class,post_class_Id,post_title,post_content,to_char(post_time,'yyyy-mm-dd') post_time,to_char(post_upDate,'yyyy-mm-dd') post_upDate,post_resNum  FROM post where post_Id = ? order by mem_Id";
		
		private static final String DELETE_POSTs = 
				"DELETE FROM post where post_Id = ?";
		
		private static final String DELETE_POST_RESPONSE = 
				"DELETE FROM post_Response where res_Id = ?";	
		private static final String UPDATE = 
				"UPDATE post_Response set mem_Id=?, post_Id=? post_Response_content=?, post_time=? post_Response_upDate=? where res_Id = ?";
	...
	public void delete(String res_Id) {
		...
		try {
			...
			System.out.println("刪除文章編號" + post_Id + "時,共有回應" + updateCount_POST_RESPONSEs
					+ "文章同時被刪除");
